...
public class ProfessionalManagementSystem extends JFrame {
    ...
    private void openAddStudentDialog() {
        ...
        btnSave.addActionListener(e -> {
            ...
            try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
                ...
                pstmt.setDouble(5,
                        Double.parseDouble(txtBill.getText().trim().isEmpty() ? "0" : txtBill.getText().trim()));
                pstmt.setString(6, txtDate.getText().trim());

                pstmt.executeUpdate();
                JOptionPane.showMessageDialog(dialog, "Success: Record inserted directly into MySQL Database schema.",
                        "Database Update Complete", JOptionPane.INFORMATION_MESSAGE);
                dialog.dispose();

            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(dialog, "JDBC Error: Connection failed.\nDetails: " + ex.getMessage(),
                        "Database Connection Fault", JOptionPane.ERROR_MESSAGE);
            } catch (NumberFormatException ex) {
                JOptionPane.showMessageDialog(dialog, "Error: Bill value numeric mapping failed.", "Type Parsing Fault",
    ...
    private void openFeesDialog() {
        ...
        gbc.gridx = 0;
        gbc.gridy = 0;
        formPanel.add(new JLabel("Target Student Name:"), gbc);
        gbc.gridx = 1;
        formPanel.add(txtSearchName, gbc);
        gbc.gridx = 0;
        gbc.gridy = 1;
        formPanel.add(new JLabel("Amount Received (INR):"), gbc);
        gbc.gridx = 1;
        formPanel.add(txtPaidAmt, gbc);
        gbc.gridx = 0;
        gbc.gridy = 2;
        gbc.gridwidth = 2;
            ...
            try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
                ...
                if (rs.next()) {
                    ...
                    JOptionPane.showMessageDialog(dialog,
                            "No candidate match located inside MySQL row queries matching search text.",
                            "Profile Not Found", JOptionPane.WARNING_MESSAGE);
                }
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(dialog, "Query Error: " + ex.getMessage(), "Database Error",
                        JOptionPane.ERROR_MESSAGE);
            }
        });
        ...
        searchBtn.addActionListener(e -> {
            ...
            try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD);
                ...
                if (matchFound) {
                    ...
                    resultDisplay
                            .setText("❌ QUERY COMPLETE: Zero matching data records found inside MySQL storage tables.");
                }
            } catch (SQLException ex) {
    ...
    public static void main(String[] args) {
        try {
            // FORCES JAVA RUNTIME SYSTEM TO INITIALIZE DYNAMIC JDBC MYSQL DRIVER CLASSPATH
            // ENGINES
